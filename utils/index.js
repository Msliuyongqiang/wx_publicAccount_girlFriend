...
const getValueByMonth = () => {
 ...
 console.log(jsonData[monthsList[currentMonth]],'jsonData[monthsList[currentMonth]] == "null"');

    if (jsonData[monthsList[currentMonth]] == null) {
        const value = jsonData[monthsList[currentMonth - 1]];
        if (currentDay <= value) {
            return `上次经期开始时间${currentMonth - 1}.${value}，预计于 ${value - currentDay} 天后到来！`;
        } else {
            return `上次经期开始时间${currentMonth - 1}.${value}，较上月已推迟 ${currentDay - value} 天！`;
        }
    } else {
...
const getLoveWords = () => {
    return new Promise((resolve, reject) => {
        ...
        const apiKey = '4fe871b8fcce280d004c6e5766c2e3aa';
